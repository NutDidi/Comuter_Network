...
bool recv_name(char *File_name)
{
	...
	while(1)
	{
		...
		if((recvname.Check(&recvHeader)) && (recvname.Flags == 48) && (string(recvname.Data) == "Exit") && (recvname.seq ==lastrecvSEQ + 1) && (recvname.ack == lastsendSEQ))
		{
			printTime();
...
void recv_content(char* &File_content,int& curlen)
{
	...
	while(1)
	{
		if(curwin>0)
		{
			...
			//非尾
			if((recvcontent.Check(&recvHeader)) && (recvcontent.Flags == 16) && recvcontent.seq == lastrecvSEQ + 1 )
			{
				packnum++;
			...
			{
				//尾部
				if( (recvcontent.Check(&recvHeader)) && (recvcontent.Flags == 24) && recvcontent.seq == lastrecvSEQ+1  && recvcontent.ack <= lastsendSEQ)
				{
				...
				else
				{
					...
					{					
						// int s1 = sendto(Server, (char*)&lastsendQ.front(), sizeof(struct Message), 0, (sockaddr*)&Client_addr, l);
						
						// printTime();
						// std::cout<<"[ Log ] Resend File Data Block Ack"<<endl;
						// printMess(lastsendQ.front(),SEND);

						// Message F =lastsendQ.front();
						// lastsendQ.pop();
						// lastsendQ.push(F);

						// int s2 = sendto(Server, (char*)&lastsendQ.front(), sizeof(struct Message), 0, (sockaddr*)&Client_addr, l);
						// printTime();
						// std::cout<<"[ Log ] Resend File Data Block Ack"<<endl;
						// printMess(lastsendQ.front(),SEND);
						...
						// lastsendSEQ =lastsendQ.front().seq;		
						int s1 = sendto(Server, (char*)&lastsend, sizeof(struct Message), 0, (sockaddr*)&Client_addr, l);
