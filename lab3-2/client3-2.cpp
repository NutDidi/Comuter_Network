...
void send_name(char* name, bool esc)
{
	...
	while(1)
	{
		...
		if (r != SOCKET_ERROR && recvname.Flags == 48 && recvname.Check(&recvHeader) &&  recvname.ack == lastsendSEQ  )
		{
			printTime();
...
void send_content(char* content,int contentlen,int i,bool k)//是否为尾
{
	...
	while(1)
	{
		...
		if(lastsendSEQ - lastrecvACK < Max_window)
		{
			if(!k)//非尾部
			{
				...
				sendcontent.Winsize = Max_window - (lastrecvSEQ - lastrecvACK) - 1;
